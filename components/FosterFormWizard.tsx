...
import { fosterSchema, type FosterFormData } from '@/lib/validations/foster';
import { sendFosterEmail } from '@/lib/actions/sendFosterEmail';

interface FosterFormWizardProps {
  dogId?: string;
  dogName?: string;
}
...
export function FosterFormWizard({ dogId, dogName }: FosterFormWizardProps) {
  const t = useTranslations('Foster');
  const [currentStep, setCurrentStep] = useState(0);
  const [formData, setFormData] = useState<Partial<FosterFormData>>({
    dogId,
    dogName,
  });
  ...
  const validateStep = (step: number): boolean => {
    ...
    requiredFields.forEach((field) => {
      const value = formData[field as keyof FosterFormData];
      if (!value || value === '') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await sendFosterEmail(parsed.data);
          ...
          <div className="space-y-4">
            ...
            <div className="form-control">
              ...
              <input
                type="text"
                name="fosterEndDate"
                value={(formData as any).fosterEndDate || ''}
                onChange={handleChange}
                className="input input-bordered w-full"
              />
            </div>
